# Guides

## What Guides Contain

* How to use the system (user perspective)
* How to implement features or backends (developer perspective)
* Step-by-step workflows
* References to specs for contract details

## What Guides Do NOT Contain

* Normative contracts or schemas (those belong in `specs/`)
* Architectural rationale (that belongs in `architecture/`)
* Development process details (those belong in `development/`)

## Available Guides

**[usage.md](./usage.md)** — Basic usage: installation, bootstrap, profiles, plan/apply workflow.

**[features.md](./features.md)** — How to create and maintain a feature module.

**[backends.md](./backends.md)** — How to implement a backend plugin.
