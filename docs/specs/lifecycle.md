...
The operation updates Known state from the old fully qualified resource ID to the new one only after the new link is verified.
...
6. Run preflight.
7. Persist the new operation record.
8. Execute actions sequentially.
9. Verify the post-condition of each completed action.
10. Atomically commit the verified Known-state update and resource progress.
11. Close the operation record when every action has a final status.
...
1. `create_link` and the create portion of `relocate_link`;
2. `replace_link` and `replace_ownership`;
3. the remove portion of completed `relocate_link`, `remove_link`, and `forget_missing`.
